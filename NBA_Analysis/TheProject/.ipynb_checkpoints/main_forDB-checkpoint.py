...
def main():
    player_names_df = pd.read_csv("../Csv_Data/2025Player_Names.csv", encoding="utf-8")
    create_gamelogs_directory()

    all_cleaned_logs = []

    for name in player_names_df["Player"][:3]:
        try:
            if name == "End":
                print("Ending Now")
                exit()
    ...
    if all_cleaned_logs:
        for idx, log in enumerate(all_cleaned_logs):
            ...
            file_path = f"Data/DataFrames/{player_name}_{year}DataFrame.html"
